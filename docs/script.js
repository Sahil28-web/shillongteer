...
// ✅ FIXED: Creates blank results (FR/SR = "") if today’s date is missing, no fake “45” placeholders.
function ensureTodayPlaceholders(items) {
  const sessions = ['Morning', 'Afternoon', 'Evening'];
  const today = formatTodayLabel();
  if (!today) return items;

  const existing = new Set(items.filter(r => r.Date === today).map(r => r.Session));
  const missing = sessions.filter(s => !existing.has(s));

  if (missing.length > 0) {
    missing.forEach(s => {
      items.unshift({ Date: today, Session: s, FR: "", SR: "" });
    });
  }
...
getResultsData().then(items => {
  ...
  if (allEl) {
    ...
    if (isHomeSnippet) {
      ...
      if (typeof sessionName !== 'undefined' && sessionName) {
        ...
        const prev = sessionOnly.slice(1);
        prev.forEach(r => {
          const tr = document.createElement('tr');
          tr.innerHTML = `<td>${r.Date}</td><td>${r.FR}</td><td>${r.SR}</td>`;
  ...
  // Common number box remains same
